...
export class Camera {
	...
	public static setTargetControllerForPlayer (whichPlayer: MapPlayer, whichUnit: Unit, xoffset: number, yoffset: number, inheritOrientation: boolean) {
		SetCameraTargetControllerNoZForPlayer(whichPlayer.handle, whichUnit.handle, xoffset, yoffset, inheritOrientation)
	}
...
export class CameraSetup extends Handle<camerasetup> {
	...
	public set position (pos: Position) {
		CameraSetupSetDestPosition(this.handle, pos.x, pos.y, 0)
	}

	public get position (): Position {
		return new Position(CameraSetupGetDestPositionX(this.handle), CameraSetupGetDestPositionY(this.handle))
	}
