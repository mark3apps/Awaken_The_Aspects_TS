...
export abstract class TalentTree {
	...
	private AddTalentRaw (x: number, y: number, data: TalentData): Talent {
		...
		if (this._talents[index]) {
			const existing = this._talents[index]
		...
		if (talent.isLink) { this._linkTalentIndices.push(index) }

		if (data.StartingLevel) { this.tempRankState[index] = data.StartingLevel }
	...
	public AddTalent (x: number, y: number, talentData: TalentData): Talent {
		...
		const talent = this.AddTalentRaw(x, y, talentData)
		return talent
	}
	...
	public ApplyTalentTemporary (index: number) {
		const talent: Talent = this._talents[index]
	...
	public CheckDependencyKey (requiredLevel: number | undefined, index: number, depIndex: number): { ok: boolean, error?: string, link: boolean } {
		...
		const depTalent = this._talents[depIndex]
	...
	private UpdateLinkState (index: number): number {
		...
		if (talent && talent.isLink) {
			if (talent.dependency.left) {
				...
				const requiredLevel = talent.dependency.left
				if (this._talents[depIndex] && this._talents[depIndex].isLink) { this.UpdateLinkState(depIndex) }
				const depResult = this.CheckDependencyKey(requiredLevel, index, depIndex)
				if (depResult && depResult.ok) { level++ }
			}
			if (talent.dependency.up) {
				const depIndex = index + this._columns
				const requiredLevel = talent.dependency.up
				if (this._talents[depIndex] && this._talents[depIndex].isLink) { this.UpdateLinkState(depIndex) }
				const depResult = this.CheckDependencyKey(requiredLevel, index, depIndex)
				if (depResult && depResult.ok) { level++ }
			}
			if (talent.dependency.right) {
				const depIndex = index + 1
				const requiredLevel = talent.dependency.right
				if (this._talents[depIndex] && this._talents[depIndex].isLink) { this.UpdateLinkState(depIndex) }
				const depResult = this.CheckDependencyKey(requiredLevel, index, depIndex)
				if (depResult && depResult.ok) { level++ }
			}
			if (talent.dependency.down) {
				const depIndex = index - this._columns
				const requiredLevel = talent.dependency.down
				if (this._talents[depIndex] && this._talents[depIndex].isLink) { this.UpdateLinkState(depIndex) }
				const depResult = this.CheckDependencyKey(requiredLevel, index, depIndex)
				if (depResult && depResult.ok) { level++ }
			}
			...
			if (this._tempRankState) { this._tempRankState[index] = level }
		}
	...
	public UpdateLinkStates () {
		for (const i of this._linkTalentIndices) {
	...
	public GetTalentTempState (index: number) {
		if (!this._tempRankState) { return this._rankState[index] }
