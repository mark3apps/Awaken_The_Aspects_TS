import { MapPlayer, Frame, Trigger } from 'lib/w3ts'
...
export class BasicTalentTreeViewModel {
	...
	OnTalentClicked (index: number) {
		...
		const talent = this._tree.talents[index]
		const tempState = this._tree.tempRankState[index]
	...
	Show (): void {
		...
		const xIncrem = (this.cfg.boxWidth * (1 - this.cfg.sideMargin)) / (cols + 1)
		const yIncrem = (this.cfg.boxHeight * (1 - this.cfg.verticalMargin)) / (rows + 1)
		...
		if (tree.maxTalents > this._slots.length) {
			for (let i = this._slots.length; i <= tree.maxTalents; i++) {
				// Create a new slot
				const index = i
				const slot = this._talentSlotFactory(i)
				this._slots[i] = slot
		...
		const maxTalents = this._slots.length
		...
		for (let i = 0; i < maxTalents; i++) {
			...
			const x = xPos * xIncrem - ((cols - 1) * 0.5) * xIncrem
			const y = yPos * yIncrem - ((rows - 1) * 0.5) * yIncrem
	...
	UpdateTalentSlot (slot: ITalentSlot, talent: Talent, tree: TalentTree, index: number) {
		const tempState = tree.GetTalentTempState(index)

		const depLeft = tree.CheckDependencyKey(talent.dependency.left, index, index - 1)
		const depUp = tree.CheckDependencyKey(talent.dependency.up, index, index + tree.columns)
		const depRight = tree.CheckDependencyKey(talent.dependency.right, index, index + 1)
		const depDown = tree.CheckDependencyKey(talent.dependency.down, index, index - tree.columns)
		...
		if (depLeft.ok && depUp.ok && depRight.ok && depDown.ok) {
			...
			depOk = false
			const concatErrs = (err1?: string, err2?: string): string | undefined => err1 && err2 ? err1 + ', ' + err2 : (err1 || (err2 || undefined))
		...
		const [reqOk, reqError] = tree.CalculateTalentRequirements(index, talent)
		...
		if (tempState === talent.maxRank) {
			...
			if (reqOk) { slot.state = TalentState.RequireDisabled } else if (depOk) { slot.state = TalentState.DependDisabled }
		}
	}

	OnFrameButtonClicked () {
		const frameId = Frame.fromEvent().id
