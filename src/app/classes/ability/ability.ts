...
import { Coordinate, Widget } from 'lib/w3ts/index'
import { AbilityHandle } from './AbilityHandle'
import { IAbility } from './interfaces/IAbility'
import { IAbilityCast } from './interfaces/IAbilityCast'

export class Ability extends AbilityHandle {
	constructor (ability: IAbility) {
		if (AbilityHandle.initFromAbility()) {
			super()
		} else {
			super(ability)
		}
	...
	castImmediate (): void {
		if (this.abilityType.orderId) this.unit.issueImmediateOrder(this.abilityType.orderId)
	}
	...
	cast (dest: Coordinate): void {
		if (this.abilityType.orderId) this.unit.issueOrderAtCoordinate(this.abilityType.orderId, dest)
	}
	 ...
	 */
	forEachLevelField (field: abilitybooleanlevelfield | abilitystringlevelfield | abilityreallevelfield | abilityintegerlevelfield, expression: (value: string | number | boolean) => string | number | boolean): void {
		for (let i = 0; i < this.levels; i++) {
			this.setLevelField(field, expression(this.getLevelField(field, i)), i)
		}
	}

	setLevelFieldArray (field: abilitybooleanlevelfield | abilitystringlevelfield | abilityreallevelfield | abilityintegerlevelfield, value: string[] | number[] | boolean[]): void {
		for (let i = 0; i < this.levels; i++) {
	...
	static fromSpell (cast: IAbilityCast) {
		return this.fromHandle(cast)
	}

	static override fromHandle (ability: IAbility) {
		return this.getObject(ability) as Ability
	}
}
