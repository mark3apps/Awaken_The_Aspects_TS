...
import { Widget } from "lib/w3ts/index"
...
export class UnitAbility extends UnitAbilityHandle {
	...
	castTarget(targetWidget: Widget): void {
		if (this.abilityType.orderId)
			this.unit.issueTargetOrder(this.abilityType.orderId, targetWidget)
	}

	cast(dest: Coordinate): void {
		if (this.abilityType.orderId)
			this.unit.issueOrderAtCoordinate(this.abilityType.orderId, dest)
	}
	...
	setField(
		field: abilitybooleanfield | abilitystringfield | abilityrealfield | abilityintegerfield,
	...
	setLevelField(
		...
		if (fieldType === "abilitybooleanlevelfield" && typeof value === "boolean") {
			return BlzSetAbilityBooleanLevelField(
				this.handle,
				field as abilitybooleanlevelfield,
				level,
				value
			)
		} else if (fieldType === "abilityintegerlevelfield" && typeof value === "number") {
			return BlzSetAbilityIntegerLevelField(
				this.handle,
				field as abilityintegerlevelfield,
				level,
				value
			)
			...
			return BlzSetAbilityRealLevelField(
				this.handle,
				field as abilityreallevelfield,
				level,
				value
			)
		} else if (fieldType === "abilitystringlevelfield" && typeof value === "string") {
			return BlzSetAbilityStringLevelField(
				this.handle,
				field as abilitystringlevelfield,
				level,
				value
			)
		}
