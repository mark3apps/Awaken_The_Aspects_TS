...
export class SkillTalentViewModel implements IKickTalentSlot {
	private costPreName = '|cffffc04dSkills: '
	...
	/** Calls local blocks. */
	RenderView () {
		const cost = (this._talent && this._talent.cost) || 0
		switch (this._state) {
	...
	public setCostText (pre: string, post: string) {
		this.costPreName = pre
		this.costPostName = post
	}
	...
	public SetTooltip (requirements: string | null, cost: number) {
		...
		if (t.nextRank && t.prevRank) { description = t.prevRank.description + '\n\nNext rank:\n' + description }

		text = t.name
		if (cost > 0) {
			text += '\n\n' + this.costPreName + I2S(cost) + this.costPostName + '|r '
		}
		try {
	...
	SetAsLink () {
		...
		if (this._talent && this._talent.iconEnabled && this._talent.iconEnabled !== '') {
			...
			if (this._rank && this._rank > 0) { this._view.button.image.setTexture(this._talent.iconEnabled, 0, true) }
		} else {
			this._view.button.main.visible = false
			this._view.linkIntersection.visible = true
			if (this.rank > 0) { this._view.linkIntersection.setTexture(this._cfg.activeLinkTexture, 0, true) } else { this._view.linkIntersection.setTexture(this._cfg.inactiveLinkTexture, 0, true) }
		}
	...
	public set rank (v: number) {
		...
		const tooltip = '[|cffFCD20DRank ' + rankText + '|r]'
