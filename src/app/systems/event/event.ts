...
export class Event {
  ...
  constructor(depend: IEventDepend, event: IEvent) {
    ...
    this.timer.start(this.eventInterval, false, () => {
      this._onEventStart()
    })

    this.eventDeath = new Trigger()
    this.eventDeath.addAction(() => {
      this._onEventUnitDeath()
    })
    this.onEventInit()
  }
  ...
  private _onEventStart(): void {
    const sightUnitAlliance = new Unit({ owner: this.forces.AllianceComps.getRandomPlayer(), type: this.unitTypes.DummySeer, coor: this.spawnPos })
    sightUnitAlliance.applyTimedLifeGeneric(this.eventDuration)

    const sightUnitFederation = new Unit({ owner: this.forces.FederationComps.getRandomPlayer(), type: this.unitTypes.DummySeer, coor: this.spawnPos })
  ...
  public createUnit(): void {
    if (this.allianceScore > this.federationScore) {
      this.eventUnit = new Unit({ owner: this.forces.AllianceComps.getRandomPlayer(), type: this.summonUnitType, coor: this.spawnPos })
      ...
      this.eventUnit = new Unit({ owner: this.forces.FederationComps.getRandomPlayer(), type: this.summonUnitType, coor: this.spawnPos })
